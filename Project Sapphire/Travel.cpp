...
BOOL MoveToQueue(WORD X, WORD Y)
{
	...
	{
		Print(0, 1, "ÿc4Shivaÿc0: ÿc1There are no nodes to teleport to");
...
BOOL WalkToQueue(WORD X1, WORD Y1)
{
	...
	{
		Print(0, 1, "ÿc4ÿc4Shivaÿc0: ÿc1Failed to generate path, starting point is not a valid index.");
	...
	{
		Print(0, 1, "ÿc4Shivaÿc0: ÿc1Failed to generate path, ending point is not a valid index.");
		V_FarcastTeleport = V_FarcastTeleportMemory;
		V_WalkingActive = FALSE;
		return FALSE;
	}
	...
	if (!Count)
	{
		Print(0, 1, "ÿc4Shivaÿc0: ÿc1Failed to generate path, could not generate any nodes.");
		V_FarcastTeleport = V_FarcastTeleportMemory;
		V_WalkingActive = FALSE;
		return FALSE;
	}
	...
	while (CalculateDistance((LONG)Me->pPath->xPos - V_LevelX, (LONG)Me->pPath->yPos - V_LevelY, (LONG)X - V_LevelX, (LONG)Y - V_LevelY) > 3) 
	{
		...
		while (Me->dwMode != 2 && Me->dwMode != 3 && Me->dwMode != 6) 
		{
			if ((GetTickCount() - nTimer) > 500) 
			{
				...
				if (nFail == 2) 
				{
					//Print(0, 1, "ÿc4Shivaÿc0: ÿc1Failed to reach node.");
...
BOOL TeleportTo(WORD X, WORD Y)
{
	...
	*(LPWORD)&Packet[3] = Y;
	...
	if (!Arrived)
	{
		Print(0, 1, "ÿc4Shivaÿc0: ÿc1Failed to reach target, terminating teleport sequence");
