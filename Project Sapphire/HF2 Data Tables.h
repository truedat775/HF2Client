...
*/
struct exAutoMagicTxt
{
	char szName[31];                //0x00
	BYTE _1;						//0x1F
	WORD wIndex;	                //0x20
	WORD wVersion;                  //0x22
	exItemsTxtStat hMods[3];          //0x24
...
struct exUniqueItemsTxt
{
	...
	exItemsTxtStat hStats[12];      //0x90
};

struct exSetItemsTxt
{
	...
	BYTE nAddFunc;                 //0x87
	exItemsTxtStat hStats[9];        //0x88
	exItemsTxtStat hGreenStats[10];   //0x118
};
...
struct exRunesTxt
{
	...
	WORD dwEType[3];                     //0x92
	DWORD dwRune[6];                     //0x98
	exItemsTxtStat hStats[7];              //0xB0
};

struct exPetTypesTxt
{
WORD pet_type; // 0x0
struct {
BYTE warp:1; // 0x4
...
struct exInventoryLayout //sizeof 0x14
{
	DWORD dwLeft;		//0x00
	DWORD dwRight;		//0x04
	DWORD dwTop;		//0x08
...
struct exInventoryTxt //sizeof 0xF0
{
	exInventoryLayout Inventory;		//0x00
	exInventoryLayout Grid;			//0x14
	union
		{
		struct 
		{
			exInventoryLayout RightArm;		//0x28
			exInventoryLayout Torso;			//0x3C
			exInventoryLayout LeftArm;		//0x50
			exInventoryLayout Head;			//0x64
			exInventoryLayout Neck;			//0x78
			exInventoryLayout RightHand;		//0x8C
			exInventoryLayout LeftHand;		//0xA0
			exInventoryLayout Belt;			//0xB4
			exInventoryLayout Feet;			//0xC8
			exInventoryLayout Gloves;			//0xDC
		};
		exInventoryLayout hItem[9];
	};
};
...
struct exItemStatCostTxt  //size 0x144
{
	WORD	wStat;				//0x00
	WORD 	_1;					//0x02
	struct {
...
struct exDifficultyLevelsTxt //size 0x58
{
...
struct exCubeInputItem		//size (0x08)
{
struct {
BYTE	bCodeFlag:1;			//1
BYTE	bClassFlag:1;			//2
...
struct exCubeOutputItem //size 0x54
{
...
struct exCubeMainTxt		//size 0x148
{
BYTE	bEnabled;			//0x00
BYTE	bLadder;			//0x01
BYTE	bMindiff;			//0x02
...
WORD	wVersion;			//0x12
exCubeInputItem InputItem[7];	//0x14
exCubeOutputItem OutputItem[3];//0x4C
};

struct exCharStatsTxt
{
...
struct exMonStatsTxt // size 0x1A8
{
WORD	wId;						//0x00
WORD	wBaseId;					//0x02
...
struct exItemsTxt //size = 0x1A8, Valid for Weapons, Armors, Misc.txts
{
...
struct exsgptDataTable {
	...
	exMonStatsTxt* pMonStatsTxt;		//0xA78
	...
	exMissilesTxt*	pMissilesTxt;	//0xB64
	...
	exSkillDescTxt*	pSkillDescTxt;	//0xB8C
	BYTE*	pSkillDesc;				//0xB90
	DWORD	dwSkillDescRecs;		//0xB94
	exSkillsTxt*pSkillsTxt;			//0xB98
	BYTE*	pSkills;				//0xB9C
	...
	exCharStatsTxt*	pCharStatsTxt;	//0xBC4
	DWORD	dwCharsStatsRecs;		//0xBC8
	exItemStatCostTxt*pItemStatCostTxt;//0xBCC
	...
	exSetItemsTxt* pSetItemsTxt;		//0xC18
	DWORD	dwSetItemsRecs;			//0xC1C
	BYTE*	pUniqueItems;			//0xC20
	exUniqueItemsTxt*	pUniqueItemsTxt;//0xC24
	...
	exDifficultyLevelsTxt*pDiffLvlsTxt;//0xC7C
	DWORD	dwDiffLvlsRecs;			//0xC80
	DWORD	_1;						//0xC84
	BYTE*	pCharTemp;				//0xC88
	DWORD	dwCharTempRecs;			//0xC8C
	exArenaTxt*pArena;				//0xC90
	exCubeMainTxt*	pCubeMain;		//0xC94
