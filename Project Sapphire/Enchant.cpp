...
BOOL ExecuteEnchantQueue(LPENCHANTINFO EnchInfo)
{
	...
	if (!(GetPvPFlags(EnchInfo->dwPlayerId) & PVP_ALLIED_WITH_YOU))
	{
		...
		Print(0, 4, "ÿc4Lazarusÿc0: ÿc1%s is not in your party", Unit->pPlayerData->szName);
	...
	{
		Print(0, 4, "ÿc4Lazarusÿc0: You don't have the Enchant skill");
		return FALSE;
	}
	...
	if (EnchInfo->nEnchantType == PLAYER)
	{
		...
		if (!lpTarget)
		{
			Print(0, 4, "ÿc4Lazarusÿc0: ÿc1%s is out of your range", lpTarget->pPlayerData->szName);
