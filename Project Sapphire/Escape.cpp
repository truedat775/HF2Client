...
BOOL Portal()
{
	...
	if (!Item)
	{
		Print(0, 4, "ÿc4Lazarusÿc0: ÿc1No town portal scrolls found");
		return FALSE;
	}
	...
	if (Book)
	{
		...
		if (Quantity <= 5)
			Print(0, 4, (Quantity == 2) ? "ÿc4Lazarusÿc0: ÿc1%d town portal scroll remaining" : "ÿc4Lazarusÿc0: ÿc1%d town portal scrolls remaining", Quantity - 1);
	}
...
BOOL Town()
{
	...
	if (!V_DelayedChicken && !InTown(Me) && !GetUnitState(Me, AFFECT_WHIRLWIND))
	{
		...
		if (V_TownAmplify && GetUnitState(Me, AFFECT_AMPLIFYDAMAGE))
			Chicken();

		if (V_TownBattleCry && GetUnitState(Me, AFFECT_BATTLECRY))
			Chicken();

		if (V_TownConvicted && GetUnitState(Me, AFFECT_CONVICTED))
			Chicken();

		if (V_TownDecrepify && GetUnitState(Me, AFFECT_DECREPIFY))
			Chicken();

		if (V_TownIronMaiden && GetUnitState(Me, AFFECT_IRONMAIDEN))
			Chicken();

		if (V_TownLowerResist && GetUnitState(Me, AFFECT_LOWERRESIST))
			Chicken();
	}
