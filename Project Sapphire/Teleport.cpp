...
{	
	...
	{
		for (p.x = pos.x - WALK_RANGE; p.x <= pos.x + WALK_RANGE; p.x++)
		{
			for (p.y = pos.y - WALK_RANGE; p.y <= pos.y + WALK_RANGE; p.y++)
			{
				if (!IsValidIndex(p.x, p.y))
					continue;

				if (m_pMap[p.x][p.y] < value && CalculateDistance(p, pos) <= WALK_RANGE)
				{				
					value = m_pMap[p.x][p.y];
					best = p;
				}
